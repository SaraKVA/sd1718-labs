...
static char player0_simbol = '0';

static char player1_simbol = 'X';
...
int play(int row, int column, int player) {
    ...
    board[row][column] = (player == 1) ? player1_simbol : player0_simbol;  /* Insert player symbol   */
...
int checkWinner() {
    ...
    else
    {
        ...
        for(line = 0; line <= 2; line ++)
        {
            ...
            {
                if (board[line][0]==player1_simbol)
					result = 1;
				else
					result = 0;
				break;
            ...
            {
                if (board[0][line]==player1_simbol)
					result = 1;
				else
					result = 0;
				break;
...
int trocaSimbolos(char simbol0, char simbol1){    
	pthread_mutex_lock(&mutex);
		player0_simbol = simbol0;
		player1_simbol = simbol1;
	pthread_mutex_unlock (&mutex);
}
